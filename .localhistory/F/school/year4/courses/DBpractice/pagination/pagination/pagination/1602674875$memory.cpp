...
void memory::WriteBack()
{
	...
	for (int i = 0; i < pageNum; i++) {
		if (m_pPageTable[i].modify) {
			...
			char* charStr = NULL;
			charStr = (char *)malloc(PAGE_SIZE * sizeof(char));
			fseek(pfile, m_pPageTable[i].position, SEEK_SET);
			fread(charStr, 1, PAGE_SIZE, pfile);
			fwrite(charStr, 1, PAGE_SIZE, temp);
		}
	}
	fclose(pfile);
	fclose(temp);
