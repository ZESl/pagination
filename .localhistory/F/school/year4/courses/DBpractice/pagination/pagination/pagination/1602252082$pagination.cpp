...
void pagination::on_pushButton_clicked()
{
	...
	if (m_memory->m_pPageTable[pageno].load) {
		int memno = m_memory->m_pPageTable[pageno].memno;
		m_strContent = m_memory->m_pMemory[memno].content;
	}
	else {
		char* pStr = m_memory->ReadPage(pageno);
