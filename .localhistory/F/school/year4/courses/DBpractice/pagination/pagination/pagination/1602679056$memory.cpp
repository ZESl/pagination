...
int memory::LoadFile(QString fileName)
{
	...
	if (myfile.open(QFile::ReadOnly | QIODevice::Text)) {
		...
		for (int i = 0; i < pageNum; i++) {
			m_pPageTable[i] = { i, false, false, (ULONGLONG)i*PAGE_SIZE, -1 };
		}
		return pageNum;
	}
	else {
...
char* memory::ReadPage(const int pageno)
{
	...
	if (m_pPageTable[pageno].load) {
		...
		//char pStr[PAGE_SIZE];
		//strcpy(pStr, m_pMemory[memno].content);
		//memcpy(pStr, m_pMemory[memno].content, PAGE_SIZE);
		qDebug() << "FROM MEMORY";
		qDebug() << m_pMemory[memno].content;
		return m_pMemory[memno].content;
	}
	else {
		...
		memset(buffer, 0, (char)PAGE_SIZE*sizeof(char));

		FILE* pfile = nullptr;
		fopen_s(&pfile, name, "rb");
		fseek(pfile, m_pPageTable[pageno].position, SEEK_SET);
		fread(buffer, 1, PAGE_SIZE, pfile);
		fclose(pfile);
		qDebug() << "len" << QString::number(strlen(buffer)) << endl;

		int memno = m_pPageTable[pageno].memno;
		m_pMemory[memno].no = memno;
		m_pMemory[memno].content = buffer;
		//memcpy(m_pMemory[memno].content, pStr, PAGE_SIZE);
		qDebug() << "FROM FILE";
		qDebug() << buffer <<endl;
		
		// assign page size text to a new array
		//char newStr[PAGE_SIZE] = { '\0'};
		//qDebug() << "len" << QString::number(strlen(newStr)) << endl;
		//memcpy(newStr, buffer, 4096);
		//qDebug() << "len" << QString::number(strlen(newStr))<< endl;
		return buffer;
	}	
}
...
bool memory::UpdatePage(const int pageno, string str, const int nMaxSize)
{
	...
	if (exist) {
		...
		qDebug() << "content:" << m_pMemory[no].content << endl;
	}
	/*else {
