...
int memory::ReadPage(const int pageno, char* pStr, const int nMaxSize)
{
	...
	buffer = (char*)malloc(PAGE_SIZE * char);
	for (i = 0; i < 50; i++) {
		if (m_pMemory[i].content == nullptr) {
			fseek(pfile, m_pPageTable[a].position, SEEK_SET);
			fread(buffer, 1, 4 * 1024, pfile);
			m_pMemory[i].content = buffer;
			m_pPageTable[a].memno = i;
			//free(buffer);
			break;
		}
	}
