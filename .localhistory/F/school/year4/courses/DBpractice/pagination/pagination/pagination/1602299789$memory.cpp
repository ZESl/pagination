...
char* memory::ReadPage(const int pageno)
{
	...
	if (m_pPageTable[pageno].load) {
		...
		char* pStr = (char*)malloc(PAGE_SIZE * sizeof(char));
		pStr = m_pMemory[memno].content;
		return pStr;
	}
	else {
		...
		//qDebug()<< m_pPageTable[pageno].position << pStr<<endl;
		fclose(pfile);

		m_pMemory[stackTop].no = stackTop;
		strcpy(m_pMemory[stackTop].content, pStr);
		return pStr;
	}	
}
		...
		for (int i = 0; i < MEM_NUM; i++) {
			if (pageno == m_anStack[i]) {
				for (int j = i; j < stackTop; j++) {
					...
					m_pPageTable[m_anStack[j]].memno = j;
				}
